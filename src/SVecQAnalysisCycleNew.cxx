...
#include "include/SVecQAnalysisHistsNew.h"
...
void SVecQAnalysisCycleNew::begin_dataset(Context & ctx){
    // 1. set up selections:
    ...
    // 2. Set up Histograms:
    h_nocuts.reset(new SVecQAnalysisHistsNew(ctx, "NoCuts"));
    h_btag.reset(new SVecQAnalysisHistsNew(ctx, "BTag"));
    h_nobtag.reset(new SVecQAnalysisHistsNew(ctx, "NoBTag"));
    h_topsel.reset(new SVecQAnalysisHistsNew(ctx, "TopSel"));
    ...
    // this cycle on an output file containing these branches, e.g. one previously produced with SVecQAnalysisCycleNew ...
    read_example_data = string2bool(ctx.get_setting("readSVecQAnalysisData"));
...
void SVecQAnalysisCycleNew::process(EventCalc & event, Context & ctx) {
    ...
    if(read_example_data){
        cout << "SVecQAnalysis data read: int = " << event_intdata_in << "; jet pt = " << event_jetdata_in.pt() << endl;
    }
...
// as we want to run the SVecQAnalysisCycleNew directly with AnalysisModuleRunner, we have to make sure we are found by the class name.
// This is achieved by calling this macro:
REGISTER_ANALYSIS_MODULE(SVecQAnalysisCycleNew)
